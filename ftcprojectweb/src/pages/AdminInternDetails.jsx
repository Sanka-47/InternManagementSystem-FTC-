...
const AdminInternDetails = () => {
  document.title = "FCT | Intern Details";
  const { user } = useAuth();
  ...
  const fetchInterns = async () => {
    ...
    setError("");
    try {
      const data = await listAllInternDetails();
      setInterns(data);
    } catch (err) {
      console.error(err);
  ...
  const handleApprove = async (internId) => {
    try {
      const res = await approveInternDetails(internId);
      setInterns((prev) =>
        prev.map((m) => (m.id === internId ? { ...m, status: "Approved" } : m))
      );
    } catch (err) {
      console.error(err);
      alert("Failed to approve intern details");
    }
  };

  const handleReject = async (internId) => {
    try {
      const res = await rejectInternDetails(internId);
      setInterns((prev) =>
        prev.map((m) => (m.id === internId ? { ...m, status: "Rejected" } : m))
      );
    } catch (err) {
      console.error(err);
      alert("Failed to reject intern details");
    }
  };

  const getStaticUrl = (relativePath) => {
    const baseUrl = import.meta.env.VITE_API_URL.replace("/api", "");
    return `${baseUrl}/uploads/interns/${relativePath}`;
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      <nav className="sticky top-0 w-full bg-gray-800 border-b border-gray-200 py-3 flex items-center justify-end px-6">
        <div className="flex flex-col text-right">
          <div className="font-semibold text-lg text-white">
            Future Code Technology - Intern Details
          </div>
      ...
      <main className="flex-1 p-6 space-y-6">
        <Card title="All Intern Details">
          {loading ? (
            <div className="py-3">Loading Interns...</div>
          ) : error ? (
              ...
              <div className="inline-block min-w-full">
                <table className="min-w-full text-sm whitespace-nowrap">
                  <thead className="text-gray-500 text-center">
                    <tr>
                      <th className="py-2 pr-8">Intern Username</th>
                  ...
                  <tbody>
                    {interns.map((intern) => (
                      <tr key={intern.id} className="border-t text-center">
                        <td className="py-2 pr-8">{intern.intern_username}</td>
                        <td className="py-2 pr-8 font-medium">
                          {intern.full_name}
                        </td>
                        <td className="py-2 pr-8">{intern.nic}</td>
                        <td className="py-2 pr-8">{intern.email}</td>
                        <td className="py-2 pr-8">{intern.phone}</td>
                        <td className="py-2 pr-8">{intern.home_address}</td>
                        <td className="py-2 pr-8">{intern.bank_branch}</td>
                        <td className="py-2 pr-8">
                          {intern.bank_account_number}
                        </td>
                        <td className="py-2 pr-8">
                          {intern.id_front_image ? (
                            <div className="flex flex-col items-center space-y-1">
                              <button
                                type="button"
                                className="p-2 rounded-md text-green-600 hover:text-green-800 hover:bg-green-50"
                                onClick={() =>
                                  handleDownload(intern.id_front_image)
                                }
                              >
                                <Download size={18} />
                              </button>
                        ...
                        <td className="py-2 pr-4">
                          {intern.id_back_image ? (
                            <div className="flex flex-col items-center space-y-1">
                              <button
                                type="button"
                                className="p-2 rounded-md text-green-600 hover:text-green-800 hover:bg-green-50"
                                onClick={() =>
                                  handleDownload(intern.id_back_image)
                                }
                              >
                                <Download size={18} />
                              </button>
