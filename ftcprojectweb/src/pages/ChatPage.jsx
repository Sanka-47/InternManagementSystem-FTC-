...
import { listMentors, listInterns } from "../services/admin";
...
export default function ChatPage() {
  ...
  // 🔹 Fetch mentors & interns
  useEffect(() => {
    async function fetchUsers() {
      const mentorsData = await listMentors();
      setMentors(mentorsData);
      const internsData = await listInterns();
  ...
  useEffect(() => {
    if (!selectedUser) return setChatHistory([]);
  ...
  const handleSend = async () => {
    ...
    const payload = {
      receiver_id: selectedUser.id, // numeric id
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <nav className="sticky top-0 w-full bg-gray-800 border-b border-gray-200 py-3 flex items-center justify-end px-6">
        <div className="flex flex-col text-right">
          <div className="font-semibold text-lg text-white">
            Future Code Technology - Admin Chat
          </div>
      ...
      <div className="flex h-screen p-4 gap-4">
        ...
        <div className="w-1/4 border-r p-2 overflow-y-auto">
          <h2 className="font-semibold mb-2">Select Mentor / Intern</h2>

          <h3 className="text-[11px] uppercase tracking-wide text-gray-400 mt-3 mb-1">Mentors</h3>
          {mentors.length === 0 && <div>Loading mentors...</div>}
          {mentors.map((mentor) => (
            <div
              key={mentor.id}
              className={`p-2 rounded cursor-pointer text-sm ${
                selectedUser?.id === mentor.id && selectedUser?.role === "Mentor"
                  ? "bg-indigo-100 text-indigo-700 font-semibold"
                  : "hover:bg-gray-100"
              }`}
              onClick={() =>
                setSelectedUser({ id: mentor.id, name: mentor.username, role: "Mentor" })
              }
            >
              {mentor.username}
            </div>
          ...
          {interns.length === 0 && <div>Loading interns...</div>}
          {interns.map((intern) => (
            <div
              key={intern.id}
              className={`p-2 rounded cursor-pointer text-sm ${
                selectedUser?.id === intern.id && selectedUser?.role === "Intern"
                  ? "bg-indigo-100 text-indigo-700 font-semibold"
                  : "hover:bg-gray-100"
              }`}
              onClick={() =>
                setSelectedUser({ id: intern.id, name: intern.username, role: "Intern" })
              }
            >
              {intern.username}
            </div>
