...
export default function AdminComments() {
  document.title = "FCT | Admin Feedback";
  const { user } = useAuth();

  const [tasks, setTasks] = useState([]); // [{task_id, task_title, project_name, ...}]
  const [commentsByTask, setCommentsByTask] = useState({}); // { [task_id]: Comment[] }
  const [newComment, setNewComment] = useState({}); // { [task_id]: string }
  ...
  useEffect(() => {
    (async () => {
        try {
        setLoading(true);
        const rows = await listDoneTasksForAdmin();
        setTasks(rows);
  ...
  const onAddComment = async (task_id) => {
    ...
    if (!text) return;
    try {
      const created = await createCommentByAdmin({ task_id: task_id, comment_text: text });
