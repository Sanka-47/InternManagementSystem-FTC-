...
import { getAllInternsForMentor, getAllAdminsForMentor } from "../services/mentor";
import { fetchChatHistory, sendMessageToApi } from "../services/message";
import api from "../services/api";


export default function ChatPageForMentor() {
  document.title = "FCT | Mentor Chat";
  ...
  useEffect(() => {
    async function fetchUsers() {
      try {
        const internsData = await getAllInternsForMentor();
        setInterns(internsData);
        const adminsData = await getAllAdminsForMentor();
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <nav className="sticky top-0 w-full bg-gray-800 border-b border-gray-200 py-3 flex items-center justify-end px-6">
        <div className="flex flex-col text-right">
          <div className="font-semibold text-lg text-white">Future Code Technology - Mentor Chat</div>
      ...
      <div className="flex h-screen p-4 gap-4">
        ...
        <div className="w-1/4 border-r p-2 overflow-y-auto">
          ...
          <h3 className="text-[11px] uppercase tracking-wide text-gray-400 mt-3 mb-1">Interns</h3>
          {interns.length === 0 && <div className="text-gray-500">Loading interns...</div>}
          {interns.map((intern) => (
            <div
              key={`intern-${intern.id}`}
              className={`p-2 cursor-pointer rounded text-sm ${
                selectedUser?.id === `intern-${intern.id}` ? "bg-indigo-100 text-indigo-700 font-semibold" : "hover:bg-gray-100"
              }`}
              onClick={() => setSelectedUser({ id: `intern-${intern.id}`, name: intern.username, role: "Intern" })}
            >
              {intern.username}
            </div>
          ))}
        ...
        <div className="flex-1 flex flex-col">
          <h2 className="font-semibold mb-2">
            Message with: {selectedUser ? selectedUser.name : <span className="text-gray-400">No one selected!</span>}
          </h2>
