...
export default function InternSidebar() {
  ...
  const handleLogout = () => {
    Swal.fire({
      title: "Are you sure?",
      text: "Do you need to logout from intern panel?",
      icon: "warning",
  ...
  return (
    <aside className="hidden md:flex md:flex-col w-64 bg-white border-r border-gray-200 p-4 gap-2 sticky top-0 h-screen">
      ...
      <div className="flex-1 overflow-y-auto pr-1">
        ...
        {navItem("/intern", "Dashboard", <MdDashboard className="text-xl" />)}

        <div className="text-[11px] uppercase tracking-wide text-gray-400 px-1 mt-4 mb-1">Sub Tasks</div>
        {navItem("/intern/kanban", "Kanban Board", <MdViewKanban className="text-xl" />)}

        <div className="text-[11px] uppercase tracking-wide text-gray-400 px-1 mt-4 mb-1">Attendance and Leaves</div>
        {navItem("/intern/attendance", "Mark Attendance", <MdCalendarToday className="text-xl" />)}
        {navItem("/intern/leaves", "Request Leave", <MdEventBusy className="text-xl" />)}

        <div className="text-[11px] uppercase tracking-wide text-gray-400 px-1 mt-4 mb-1">Chat</div>
        {navItem("/intern/chat", "Intern Chat", <MdComment className="text-xl" />)}

        <div className="text-[11px] uppercase tracking-wide text-gray-400 px-1 mt-4 mb-1">Profile</div>
        {navItem("/intern/details", "Intern Profile", <MdAccountCircle className="text-xl" />)}
      </div>
